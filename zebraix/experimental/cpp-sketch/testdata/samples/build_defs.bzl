    ...
    native.genrule(
        ...
        for FILESET in $(location :fontfiles) ; do
         ...
         """ + " ".join([
                ...
                "</fontconfig>" +
                "\" > \"$${FONTCONFIG_FILE}\";",
                "$(location //base_dir/mid_dir/mid_dir/zebraix/tools:zebraix_svg)",
                located_src_file,
                "$(location " + out_dir + out_file + ")",
            ] + extra_args),
        tools = [
            "//base_dir/mid_dir/mid_dir/zebraix/tools:zebraix_svg",
        ],
...
def test_planar_svg(
    ...
    native.sh_test(
        name = name,
        srcs = ["//base_dir/mid_dir/mid_dir/zebraix/base:diff_test.sh"],
        args = [
