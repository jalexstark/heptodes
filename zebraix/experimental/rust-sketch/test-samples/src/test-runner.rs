...
// use jaywalk_base::jaywalk_graph::JaywalkGraph;
...
fn run_json_test(input_filename: &str, output_filename: &str, args: &Vec<&str>) {
    ...
    let input_full_path = format!("tests/goldeninputs/{}", input_filename);
    ...
    // let inbound_proto = zebraix_proto::read_file(File::open(input_full_path).unwrap()).unwrap();

    // assert!(
    //     inbound_proto.get_nodes()[0].get_label_text() == "Four legs",
...
fn test() {
    let mut mint = Mint::new("tests/goldenfiles");
    let file =
        mint.new_goldenfile_with_differ("recapture_demo.svg", Box::new(custom_diff)).unwrap();
...
//   * Convert from proto to internal.
...
fn run_one_test(input_filename: &str, output_filename: &str, args: &Vec<&str>) {
    ...
    let _input_full_path = format!("tests/goldeninputs/{}", input_filename);
    // let inbound_proto = zebraix_proto::read_file(File::open(input_full_path).unwrap()).unwrap();
